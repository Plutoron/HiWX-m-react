import React, { Component } from 'react';
import { Link } from 'react-router-dom';

class OrderUserInfo extends Component {
  constructor(props) {
    super(props);
  
    this.state = {
      unlogin:false,
      selectedTime:false,
      selectedCoupon:false,
    };
  }
  componentDidMount() {
   
  }
  render() {
    return (
        <div className='OrderUserInfoBlock'>
            <div className='perLine'>
                <input placeholder='请输入姓名' />
            </div>

            <div className='perLine'>
              <div className='leftItem'>  
                  <input placeholder='请输入图片验证码' />
              </div>
              <div className='rightItem'>
                  <img src='data:image/jpeg;base64,'/>
              </div>
            </div>

            <div className='perLine'>
              <div className='leftItem'>  
                  <input placeholder='请输入短信验证码' />
              </div>
              <div className='rightItem'>
                  <span>获取验证码</span>
              </div>
            </div>


            <div className='perLine'>
              <div className='leftTitle'>  
                  预约时间
              </div>
              <div className='rightSelect'>
              {
                this.state.selectedTime ? 
                <span className='selectedSpan'>asfasdfasdfadsfasdfdas</span> :
                <span className='selectSpan'>选择预约时间</span>
              }
              </div>
            </div>

            <div className='perLine'>
              <div className='leftTitle'>  
                  优惠券
              </div>
              <div className='rightSelect'>
              {
                this.state.selectedCoupon ? 
                <span className='selectedSpan'>减100</span> :
                <span className='selectSpan'>选择优惠券</span>
              }
              </div>
            </div>

            <div className='perLine'>
              <textarea placeholder='如有其它问题请留言' />
            </div>
        </div>
    );
  }
}

export default OrderUserInfo;